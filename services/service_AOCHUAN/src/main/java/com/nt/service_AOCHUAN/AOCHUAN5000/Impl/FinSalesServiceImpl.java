...
public class FinSalesServiceImpl implements FinSalesService {
    ...
    //弹窗更新未回款
    @Override
    public void updateallw(List<FinSales> finSales, TokenModel tokenModel) throws Exception {
        for (int i = 0; i < finSales.size(); i++) {
            finSales.get(i).preUpdate(tokenModel);
            finSalesMapper.toUpDateReFundedw(finSales.get(i).getEx_rate(), finSales.get(i).getAmountreceived(), finSales.get(i).getInvoiceamount(), finSales.get(i).getBillingtime(), finSales.get(i).getCommissionamounta(), finSales.get(i).getContractnumber());
        }
    }
}
