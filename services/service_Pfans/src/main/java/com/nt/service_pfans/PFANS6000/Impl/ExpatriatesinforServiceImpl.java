...
public class ExpatriatesinforServiceImpl implements ExpatriatesinforService {
    ...
    public List<Expatriatesinfor> getexpatriatesinforthisyear(Expatriatesinfor expatriatesinfor) throws Exception {
        ...
        List<Expatriatesinfor> expatriatesinforList2 = expatriatesinforMapper.select(expatriatesinfor);
        for(int f = 0; f < expatriatesinforList2.size(); f ++) {
            if (expatriatesinforList2.get(f).getProject_name() != null
                    && expatriatesinforList2.get(f).getProject_name() != ""
                    && expatriatesinforList2.get(f).getManagerid() != null
                    && expatriatesinforList2.get(f).getManagerid() != "") {
                expatriatesinforList.add(expatriatesinforList2.get(f));
            }
        }
        //向前台返回数据的List
                ...
                for (int i = 0; i < j; i++) {
                    ...
                    if (exitime_d == null || exitime_d.after(newYearDay_d)) {
                        ...
                        if (exitime_d != null) {//退场时间不为空
                            ...
                            //退场时间为空：只需判断入场时间是否为今年，若为今年4月1号且已经入场，考勤月份=当前月份-入场月份+1
