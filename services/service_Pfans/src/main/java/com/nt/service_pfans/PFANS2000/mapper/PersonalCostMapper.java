...
public interface PersonalCostMapper extends MyMapper<PersonalCost>{
    ...
    PersonalCostBmSum getPersonalCostBmSumGs(@Param("groupid") String groupid,@Param("yearsantid") String yearsantid);

    @Select("select LTRANK from personalcost where LTRANK is not null and CENTERID = #{groupid} and yearsantid = #{yearsantid} GROUP BY LTRANK order by LTRANK")
    ...
    List<String> getGroupinRanksGp(@Param("groupid") String groupid,@Param("yearsantid") String yearsantid);

    @Select("SELECT DEPARTSHORT AS departshortBmSum,LTRANK AS exrankBmSum,count( userid ) AS peopleBmSum,sum( round( MONTHLYSALARY, 2 ) ) AS monthBmSum,sum( round( BASICALLYANT, 2 ) ) AS basicBmSum,sum( round( RESPONSIBILITYANT, 2 ) ) AS balityBmSum,sum( round( TOTALSUBSIDIES, 2 ) ) AS totalsubsidiesBmSum,sum( round( MONTHLYBONUS, 2 ) ) AS monthlybonusBmSum,sum( round( ANNUALBONUS, 2 ) ) AS annualbonusBmSum,sum( round( TOTALWAGES, 2 ) ) AS totalwagesBmSum,sum( round( TRADEUNIONFUNDS, 2 ) ) AS tradeunionfundsBmSum,sum( round( SBGSAJ, 2 ) ) AS sbgsajBmSum,sum( round( GJJGSFDAJ, 2 ) ) AS gjjgsfdajBmSum,sum( round( APTOJU, 2 ) ) AS aptojuBmSum,sum( round( SBGSJM, 2 ) ) AS sbgsjmBmSum,sum( round( GJJGSFDJM, 2 ) ) AS gjjgsfdjmBmSum,sum( round( JUTOMA, 2 ) ) AS jutomaBmSum,sum( round( OVERTIMEPAY, 2 ) ) AS overtimepayBmSum from personalcost where CENTERID = #{groupid} and LTRANK = #{rank} and yearsantid = #{yearsantid}")
    ...
    PersonalCostBmSum getPersonalCostSumGp(@Param("groupid") String groupid, @Param("rank") String rank,@Param("yearsantid") String yearsantid);
    //公司
